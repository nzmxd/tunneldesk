...
| Linux x64 | 支持 | 便携 `.tar.gz` + `.deb` + AppImage | 主要 Linux 目标 |
...
普通配置编辑不需要管理员权限。Ubuntu/Linux 上推荐安装 `.deb` 包：主程序以普通用户运行，启动、停止或修复 hosts 映射时，通过 polkit 弹出系统授权，只让最小权限 helper 修改 `/etc/hosts`。
...
AppImage 仍可作为便携版本使用，但不会自动安装系统级 polkit helper。需要修改 hosts 时，请优先使用 `.deb` 包。
...
- `logs\`：应用滚动日志。
- `backups\`：应用直接写 hosts 时创建的 hosts 备份。Linux polkit helper 写 hosts 时，备份位于 `/var/lib/tunneldesk/backups`。
...
Ubuntu/Linux 构建 `.deb` 和 AppImage：
...
- 打包任务，在 `main` 分支 push 和手动触发时运行：
  ...
  - Linux x64：`x86_64-unknown-linux-gnu`，便携二进制 `.tar.gz`、`.deb` 和 AppImage。
