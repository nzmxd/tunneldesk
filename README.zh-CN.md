...
TunnelDesk 是一个面向 Windows/Linux 的桌面 SSH 隧道管理工具，主要服务于本地开发场景。它让本地应用继续使用真实的服务域名和端口，同时由 TunnelDesk 把选中的域名映射到本机回环地址，并通过一个或多个 SSH 跳板机转发流量。
...
- CI 打包覆盖 Windows x64/x86/ARM64 与 Linux x64/x86。
...
| Linux x86 | 实验性 | 二进制文件 + `.deb` | GitHub 托管 runner 的 32 位 WebKitGTK 支持有限 |
| macOS | 暂未配置 | 无 | 后续需要时可以补充 |
...
Windows 上，`launchAtLogin` 会写入当前用户的 `Run` 注册表项。Linux 上，它会写入 `~/.config/autostart/TunnelDesk.desktop`。
...
- 打包任务，在 `main` 分支 push 和手动触发时运行：
  ...
  - Linux x86：`i686-unknown-linux-gnu`，release 二进制文件 + `.deb`，标记为实验性。
